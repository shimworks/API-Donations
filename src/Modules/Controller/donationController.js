const { newDonation, getDonationsByDonor, updateDonationValue, deleteDonorDonation } = require("../Services/donationServices");
...
const deleteDonation = async (req, res) => {
  const donor = req.user;
  const donationId = req.params.id;
  const result = await deleteDonorDonation(donationId, donor);
  return res.status(result.status).json({ message: result.message });
}
